...
exports.SirvlogServer = SirvlogServer = function (opts) {
    ...
    this.messages = persistentObject.load(path.resolve(this.options.runtimeDir, '.sirvlog.queue'), {
        queue: []
    });
...
SirvlogServer.prototype.startTCPServer = function (name, protocol, port) {
    ...
    self.servers[name] = net.createServer(function (sc) { //'connection' listener
        ...
        protocol.createProtocol(sc).
            on('receive', function (message) { // log message received
                ...
                self.processMessage(message, filterCtx);
    ...
    self.servers[name].listen(port, self.options.network.bind, function () { //'listening' listener

        console.log(logPrefix + 'Listening on ', self.servers[name].address());
    });
...
SirvlogServer.prototype.bulkIndex = function (facility) {
    ...
    if(self.bulkOperationPending || !self.indexName){
        return;
    }

    self.bulkOperationPending = true;

    var count = self.messages.queue.length;

    if(count){
        var msgs = self.messages.queue;

        if(count > 5000){ // limit to 5000 per single bulk operation
            count = 5000;
            msgs = self.messages.queue.slice(0, count);
        }

        self.elasticClient.bulk(msgs, function(err, res){
            if (err) {
                console.error('ElasticSearch.bulk() failed: ' + err, 'Messages in queue: ', self.messages.queue.length);
            } else {
                self.messages.queue.splice(0, count);
            }
            if(self.messages.queue.length > self.options.queue.overflowLimit){
                console.log('WARNING: Queue overflow! Dropping', (self.messages.queue.length - self.options.queue.overflowLimit), 'messages to the', self.options.queue.overflowLogFile );

                var excessive = self.messages.queue.splice(0, self.messages.queue.length - self.options.queue.overflowLimit);
                var excessiveLog = [];
                excessive.forEach(function(el){
                    excessiveLog.push(JSON.stringify(el.create.data));
                })
                fs.appendFile(self.options.queue.overflowLogFile, excessiveLog.join('\n'), { encoding: 'utf8'});
            }
            self.bulkOperationPending = false;
        });
    } else {
        self.bulkOperationPending = false;
    }
...
SirvlogServer.prototype.getIndexName = function () {
    ...
    if (/(weekly|daily)/.test(this.options.elasticsearch.index.rotate)) {
        ...
        var day = d.getDate();
        newIndexName +=  '-' + (m < 10 ? ('0' + m) : m) + '-' + (day < 10 ? ('0' + day) : day);
    } else {
        newIndexName += '-' + (m < 10 ? ('0' + m) : m);
    }
...
SirvlogServer.prototype.rotateIndex = function () {
    ...
    var newIndexName = self.getIndexName();

    self.elasticClient.indexExists(newIndexName, function(err, exists){
        if (err) {
        ...
        if(!exists){
            self.elasticClient.createIndex(newIndexName, {
                'settings': self.options.elasticsearch.index.settings,
                'mappings': {
                    message : {
