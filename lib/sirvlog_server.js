...
SirvlogServer.prototype.bulkIndex = function (facility) {
    ...
    if(this.bulkOperationPending){
        return;
    }

    this.bulkOperationPending = true;

    var count = this.messagesCache.length;

    if(count){
        var msgs = this.messagesCache;

        if(count > 5000){ // limit to 5000 per single bulk operation
            count = 5000;
            msgs = this.messagesCache.slice(0, count);
        }

        this.elasticClient.bulk(msgs, function(err, res){
            if (err) {
                console.error('ElasticSearch.bulk() failed: ' + err, 'Messages in queue: ', this.messagesCache.length);
            } else {
                this.messagesCache.splice(0, count);
            }
            this.bulkOperationPending = false;
        }.bind(this));
    } else {
